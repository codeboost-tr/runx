...
export async function resolveMarketplaceSkill(
  ...
  const candidates = adapters.filter(
    (adapter) => adapter.source === parsed.source || aliasesFor(adapter.source).includes(parsed.source),
  );
...
function aliasesFor(source: string): readonly string[] {
  return source === "fixture-marketplace" ? ["fixture"] : [];
}
