...
| P1.14 | Open | Current graph uses matching quote/reserve/spend bounds; P1.8 now proves the CLI path reaches native authority admission. | Spend above reserved bounds is rejected before mock execution. | Missing quote-drift graph fixture that changes the spend binding relative to reserved bounds and asserts the same pre-rail stop. | Add drifted spend-binding fixture using the proven P1.8 core-backed harness path. |
